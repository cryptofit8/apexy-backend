...
import { infura } from "../ContractRoute/contract";

// Create a new instance of the Express Router
const AdminRouter = Router();

const checkAdmin = async (id: string) => {
  const result = await User.findOne({ _id: id, role: 1 })
    ...
    if (checkAdmin(userId)) {
      ...
      if (exist) {
        ...
        try {
          ...
          const contract = new web3.eth.Contract(APEX_ABI, USDT_ADDRESS[chainId]);
...
// @route    POST api/contract/setfee
// @desc     setfee
// @access   Private
AdminRouter.post("/setfee", authMiddleware, async (req: AuthRequest, res) => {
  try {
    const { amount, chainId } = req.body;
    const { id: userId } = req.user;
    if (checkAdmin(userId)) {
      ...
      if (exist) {
        ...
        try {
          ...
          if (rept.status.toString() === "1") {
            ...
            console.log(`Set fee transaction failed`);
            return res.status(400).json({ err: "Set fee transaction failed" });
          }
        } catch (e: any) {
          console.error(e);
          return res
...
// @access   Private
AdminRouter.post("/setstakingperiod", authMiddleware, async (req: AuthRequest, res) => {
  try {
    const { amount, chainId } = req.body;
    const { id: userId } = req.user;
    if (checkAdmin(userId)) {
      ...
      if (exist) {
        ...
        try {
          ...
          if (rept.status.toString() === "1") {
            ...
            return res.status(400).json({ err: "Set stakingperiod transaction failed" });
          }
        } catch (e: any) {
          console.error(e);
          return res
...
// @desc     setclaimperiod
// @access   Private
AdminRouter.post("/setclaimperiod", authMiddleware, async (req: AuthRequest, res) => {
  try {
    const { amount, chainId } = req.body;
    const { id: userId } = req.user;
    if (checkAdmin(userId)) {
      ...
      if (exist) {
        ...
        try {
          ...
          if (rept.status.toString() === "1") {
            ...
            return res.status(400).json({ err: "Set claimperiod transaction failed" });
          }
        } catch (e: any) {
          console.error(e);
          return res
...
// @access   Private
AdminRouter.post("/addhistory", authMiddleware, async (req: AuthRequest, res) => {
  try {
    const { amount, chainId } = req.body;
    const { id: userId } = req.user;
    if (checkAdmin(userId)) {
